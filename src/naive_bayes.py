...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        class_priors: Dict[int, torch.Tensor] = {
            0: (labels == 0).sum().float() / len(labels),
    ...
    def estimate_class_posteriors(
        ...
        log_posteriors = torch.zeros(2)
        log_posteriors[0] = torch.log(self.class_priors[0]) + (torch.log(self.conditional_probabilities[0]) * feature).sum()
        log_posteriors[1] = torch.log(self.class_priors[1]) + (torch.log(self.conditional_probabilities[1]) * feature).sum()
